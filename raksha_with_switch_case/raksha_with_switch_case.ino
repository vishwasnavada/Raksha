...
void setup()
{
 // pinMode(interruptPin, INPUT_PULLUP); //Set pin d2 to input using the builtin pullup resisto
...
void temp()
{
  ...
  thermistor_resistance = ( ( 3.301 * ( 10 / output_voltage ) ) - 10 ); /* Resistance in kilo ohms */
  ...
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
  display.print("Temperature in Celsius = ");
  display.print(temperature);
  Serial.print(temperature);
  y[x] = map(temperature, 0, 1023, HEIGHT + 10, 0); // Leave some screen for the text.....
  drawY();
  x++;
  display.setTextSize(0);
  display.setCursor(0, 52);
  display.print("Temperature in Farenheit = ");
